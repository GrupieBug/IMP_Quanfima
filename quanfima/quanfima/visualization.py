...
def plot_polar_heatmap(data, name, interp_factor=5., color_limits=False,
    ...
    log_scale : float
        Specifies if the heatmap should be in the logarithmic scale.
...
def create_pie_chart(data, rngs, colors=['#244268', '#426084', '#67809F', '#95A9C1', '#C6D2E0'],
    ...
    zebra_color : tuple
        ...
        the specified index of the region (True, reg_index).
...
def plot_3d_orientation_map(name, lat_data, azth_data, radius_structure_elem=1,
    ...
    azth, lat = azth_data[rmin:rmax, cmin:cmax, zmin:zmax], np.abs(lat_data[rmin:rmax, cmin:cmax, zmin:zmax])
...
def plot_color_wheel(name, output_dir=None, dpi=500, xlabel='Elevation',
    ...
    """Plots the color wheel for visualizations of 3D orientation.
