...
def prepare_data(data, dilate_iterations=1, sigma=0.5):
    ...
    dilate_iterations : integer
        Indicates the number of iterations for thickening the skeleton.
...
def calculate_tukey_posthoc(df, column, type_column='type', verbose=True, write=False, name=None, output_dir=None):
    ...
    ----------
    df : pandas DataFrame
        Contains the table of values and corresponding types or classes.
    ...
    Returns
    -------
    dict : sample types and p-values
        The dict of sample types and corresponding p-values.
    """
