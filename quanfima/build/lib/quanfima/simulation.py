...
def mkfiber(dims_size, length, radius, azth, lat, offset_xyz):
    ...
    Computes a fiber of specified `length`, `radius`, oriented under azimuth `azth` and
    ...
    # Draw circle perpendicular to the directional vector
    ...
    # Propagate the circle profile along the directional vector
    ...
    pt_filter = lambda pt: np.all(np.greater_equal(pt, (0, 0, 0))) and np.all(np.less(np.array(pt), dims_size))
    n_slices = 0
    fiber_pts = None
    for pts in slices_pts:
        ...
        if len(slice_pts) > 0:
            n_slices += 1
            fiber_pts = slice_pts if fiber_pts is None else np.concatenate((fiber_pts, slice_pts))
...
def simulate_fibers(volume_shape, n_fibers=1, radius_lim=(4, 10), length_lim=(0.2, 0.8),
    ...
    (volume, lat_ref, azth_ref, diameter, n_generated, elapsed_time) : tuple of arrays and numbers
        ...
        azimuth angles at every fiber point, the volume of diameters at every fiber point,
            ...
            if not intersect:
                if np.any(volume[z_gap, y_gap, x_gap]):
                    ...
                    if n_fails == max_fails:
                        print("The number of fails exceeded. Generated {} fibers".format(n_generated))
...
def generate_datasets(volume_size=(512, 512, 512), n_fibers=50, radius_lim=(4, 10),
    ...
    Simulates a number of fiber configurations specified in `params` with `n_fibers` of the
    ...
    median_rad : integer
        Indicates the radius of median filter to fill holes occurred due to rounding of
    ...
    np.save(os.path.join(output_dir, 'dataset_fibers_n{}_r{}_{}_g{}_{}_mr{}_i{}.npy'.format(n_fibers, radius_lim[0],
                                                                                            radius_lim[-1], gap_lim[0],
                                                                                            gap_lim[-1], median_rad,
                                                                                            int(intersect))), out)
...
def simulate_particles(volume_shape, n_particles=1, radius_lim=(3, 30), max_fails=10):
    ...
    while n_generated < n_particles and n_fails < max_fails:
        ...
        if np.any(volume[z, y, x]):
            ...
            if n_fails == max_fails:
                print("The number of fails exceeded. Generated {} particles".format(n_generated))
...
def generate_particle_dataset(volume_size=(512, 512, 512), n_particles=500,
    ...
    data, diameter_data, n_gen_particle, elapsed_time = simulate_particles(volume_size, n_particles=n_particles,
                                                                           radius_lim=radius_lim, max_fails=max_fails)
...
def additive_noise(params, noise_lvl, smooth_lvl, use_median=True, median_rad=3):
    ...
    def median_fltr(data_, footprint):
        out = np.zeros_like(data_, dtype=np.uint8)

        for i in range(data_.shape[0]):
            out[i] = filters.rank.median(data_[i], selem=footprint)

        return out

    def threshold_dataset(data_):
        data_seg = np.zeros_like(data_, dtype=np.uint8)
        data_8bit = exposure.rescale_intensity(data_, in_range='image',
    ...
    precision, recall, fbeta_score, support = metrics.precision_recall_fscore_support(data_ref_skel.flatten(),
    ...
    dataset_outpath = os.path.join(output_dir, 'dataset_noised_fibers_{}_nl{}_sl{}.npy'.
