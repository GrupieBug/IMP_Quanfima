...
def cast_ray(theta, y0, x0, fiber_mask, ray_len=100):
    ...
    length of `ray_len`. The ray is cast towards to opposite directions, thus two points
    ...
    ----------
    theta : float
        Indicates the angle under which the ray is cast.
...
# Parameter orientation used to be named angle but was defined as orientation in the contract. Changed to orientation
def scan_fiber_thickness(orientation, patch, angular_step=1., tilt_range=range(-5, 6), ray_len=100):
    ...
    for tilt_amount in tilt_range:
        theta = orientation + pi2 + tilt_amount * rad_p_deg
...
def estimate_fiber_properties(fiber_mask, fiber_skel, padding=25, window_radius=12,
    ...
    by scanning a mask of fibers in a specified angular range.
    ...
    padded_fiber_skel = np.pad(fiber_skel, pad_width=(padding,), mode='constant', constant_values=0)
    padded_fiber_mask = np.pad(fiber_mask, pad_width=(padding,), mode='constant', constant_values=0)
    ...
    def calc_orientation_pca(patch):
        ...
        orientations = [r.orientation for r in rgns if r.label == dists[0][0]]
        centroids = [r.centroid for r in rgns if r.label == dists[0][0]]

        orientation_ = orientations[0]
        y0_, x0_ = centroids[0]

        return orientation_, y0_, x0_
    ...
    def calc_orientation_tensor(patch):
        ...
        orientation_ = math.atan2(*v[:, np.argmax(w)])
        y0_, x0_ = patch.shape[0] / 2, patch.shape[1] / 2

        return orientation_, y0_, x0_

    for i, (yy, xx) in enumerate(zip(ycords, xcords)):
        patch_skel = padded_fiber_skel[(yy - window_radius):(yy + window_radius + 1), (xx - window_radius):
                                                                                      (xx + window_radius + 1)]

        patch_mask = padded_fiber_mask[(yy - window_radius):(yy + window_radius + 1), (xx - window_radius):
                                                                                      (xx + window_radius + 1)]
    ...
    output_orientation_map = output_orientation_map[padding:-padding, padding:-padding]
    output_diameter_map = output_diameter_map[padding:-padding, padding:-padding]
    clear_fiber_skel = clear_padded_fiber_skel[padding:-padding, padding:-padding]
...
def estimate_fourier_orientation(data, grid_shape=(2, 2), sigma=2., zoom=1., order=3):
    ...
    zoom : float
        Indicates the up-scaling factor of each block before applying 2D FFT.

    order : str
        Indicates the order of interpolation used in the up-scaling procedure.
...
def _filter_coords(data, ws, th_nval=0.8):
    """Filters coordinates of 3D local windows by number of non-zero values specified by `th_nval`.
    """
    ws2 = ws / 2

    def check_point(data_, pt, ws2_, th):
        size = max(data_.shape)
        lim0 = pt - ws2_
        lim1 = pt + ws2_
        ...
        vol = data_[z0:z1, y0:y1, x0:x1]
        # nz_vals = np.count_nonzero(vol)

        if np.count_nonzero(vol) < ws2_ * 2. * th:
            return False
...
def object_counter(stack_binary_data):
    ...
    (objects_stats, labeled_stack) : tuple
        The tuple of a DataFrame object of counted particles and the labeled 3D data.
    """
...
def calc_porosity(data):
    ...
    mats = np.unique(data)
    out = {'Material {}'.format(m): 1. - (float(data[data == m].size) / total_volume) for m in mats[mats > 0]}
    return out
